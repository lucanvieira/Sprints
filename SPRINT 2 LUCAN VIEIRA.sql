...
SELECT COMPANY.ID, avg(TRANSACTION.AMOUNT) as media
	from company
	join transaction on transaction.company_id=company.id
	WHERE DECLINED LIKE "0"
	GROUP BY 1
...
from company, transaction
where transaction.company_id=company.id and company.id is not null and transaction.company_id is not null;
...
SELECT distinct date(timestamp) as Fecha, sum(amount) 
from transaction 
group by Fecha
order by 2 desc 
limit 5;
...
select company.country, avg(amount) as media_de_ventas
from company
join transaction on transaction.company_id=company.id
group by 1
order by 2 desc;
